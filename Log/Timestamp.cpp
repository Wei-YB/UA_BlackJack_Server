...
bool PROJECT_NAME::operator<(const Timestamp& lhs, const Timestamp& rhs) {
...
double PROJECT_NAME::operator-(const Timestamp& lhs, const Timestamp& rhs) {
    return static_cast<double>(lhs.microSecond_ - rhs.microSecond_) / MicroSecondsPerSecond;
}

Timestamp PROJECT_NAME::operator+(const Timestamp& stamp, double seconds) {
    return Timestamp(stamp.microSecond_ + static_cast<int64_t>(seconds * MicroSecondsPerSecond));
}

Timestamp& PROJECT_NAME::operator+=(Timestamp& stamp, double seconds) {
